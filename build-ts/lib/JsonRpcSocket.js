 ...
 * @type {object}
 */
export const RPC_ERRORS = Object.assign(Object.assign({}, jsonrpc.ERRORS), { INTERNAL_SERVER_ERROR: {
        code: -32000,
...
export default class JsonRPCSocket extends EventEmitter {
        ...
        switch (message.type) {
            ...
            case jsonrpc.MESSAGE_TYPES.BATCH: {
                ...
                let results = await Promise.all(batch.map((msg) => {
                    ...
                    if (msg.type === jsonrpc.MESSAGE_TYPES.NOTIFICATION ||
                    ...
                    else if (msg.type === jsonrpc.MESSAGE_TYPES.REQUEST ||
                        msg.type === jsonrpc.MESSAGE_TYPES.INTERNAL_REQUEST) {
                        return this._handleIncomingRequest(msg);
                    }
                    else if (msg.type === jsonrpc.MESSAGE_TYPES.ERROR ||
            ...
            default:
                throw new Error(`Unsupported type of message ${message.type}. ` +
    ...
    async _handleIncomingRequest(message) {
        ...
        return await waitForResponse.then((result) => {
            if (result.type === "error") {
                return jsonrpc.createError(message.payload.id, result.data.error, result.data.additionalData);
            }
            else {
                return jsonrpc.createResponse(message.payload.id, result.data);
            }
        }, () => jsonrpc.createError(message.payload.id, RPC_ERRORS.INTERNAL_SERVER_ERROR));
    }
    /**
     * Call remote method
        ...
        return new Promise((resolve, reject) => {
            ...
            const request = {
                timer: isFinite(waitTime)
                    ? setTimeout(() => request.promise.reject(new TimeoutError({ method, params })), waitTime)
                    : null,
                promise: {
                    resolve: (data) => {
                        resolve(data);
