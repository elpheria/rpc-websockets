...
export function assertNotificationName(name, isInternal = false) {
    if (typeof name !== "string")
        throw new TypeError(`Notification name should be a string, ${getType(name)} passed`);
...
export default class Namespace extends EventEmitter {
    constructor(name, options) {
        ...
        this.registerInternalMethod("on", (notifications, socket) => this._updateRemoteSubscribers(true, notifications, socket));
        this.registerInternalMethod("off", (notifications, socket) => this._updateRemoteSubscribers(false, notifications, socket));
    }
    destruct() {
    ...
    addClient(socket) {
        if (!this.hasClient(socket)) {
            ...
            socket.on("rpc:notification", (notification) => {
                ...
                this.emit(`rpc:notification:${notification.method}`, notification.params, socket);
            });
            ...
            socket.on("rpc:internal:notification", (notification) => {
                ...
                this.emit(`rpc:internal:notification:${notification.method}`, notification.params, socket);
            });
    ...
    _changeSubscriptionStatus(action, isInternal, subscriptions, handler) {
        ...
        Object.entries(subscriptions).forEach(([n, h]) => {
            ...
            if (!isInternal && n.startsWith("rpc."))
                throw new Error("Notification with 'rpc.' prefix is for internal use only. " +
