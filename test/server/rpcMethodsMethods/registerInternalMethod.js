...
{
    describe(".registerInternalMethod()", () =>
    {
        it("Alias for \"registerInternalMethod()\" of given namespace", async () =>
        {
            ...
            namespace.registerInternalMethod = spy()
            server.registerInternalMethod(METHOD_NAME, METHOD_HANDLER, NAMESPACE_NAME)
            expect(namespace.registerInternalMethod).to.have.been.called.with(METHOD_NAME, METHOD_HANDLER)
        })
        ...
        it("If no namespace passed - uses \"/\" namespace by default", async () =>
        {
            ...
            const namespace = server.createNamespace(NAMESPACE_NAME)
            namespace.registerInternalMethod = spy()
            server.registerInternalMethod(METHOD_NAME, METHOD_HANDLER)
            expect(namespace.registerInternalMethod).to.have.been.called.with(METHOD_NAME, METHOD_HANDLER)
        })
        ...
        it("Creates namespace if it is not already exists", async () =>
        {
            ...
            server.registerInternalMethod(METHOD_NAME, METHOD_HANDLER, NON_EXISTING_NAMESPACE)
        ...
        it("Throws an error if passed namespace name is not a string", async () =>
        {
            ...
            {
                expect(() => server.registerInternalMethod(METHOD_NAME, METHOD_HANDLER, invalidName))
                    .to
