    ...
    describe(".sendNotification()", () =>
    {
        it("Expects notification name to be passed in first argument and notification parameters to be passed in second", async () =>
        {
            ...
            const receivedMessage = await new Promise((resolve) =>
            {
                ...
                server.sendNotification(NOTIFICATION_NAME, params)
            })
        ...
        it("Throws an error if notification name is not a string", async () =>
        {
            ...
            {
                const error = await server.sendNotification(invalidName).catch((e) => e)
        ...
        it("Throws an error if notification name starts with 'rpc.'", async () =>
        {
            ...
            {
                const error = await server.sendNotification(invalidValue).catch((e) => e)
                expect(error).to.be.instanceOf(Error)
                expect(error.message).to.be.equal(ERROR_TEXT)
            }
        })
        ...
        it("Throws an error if invalid notification parameters are passed", async () =>
        {
            ...
            for (const invalidValue of invalidValues)
            {
                const result = await server
                    .sendNotification("some_notification", invalidValue)
                    .catch((e) => e)
        ...
        it("Sends notification to all existing namespaces", async () =>
        {
            ...
            const namespaces = namespacesNames.map((namespaceName) =>
            {
                ...
                namespace.sendNotification = spy()
                return namespace
            })
            await server.sendNotification(NOTIFICATION_NAME)
            namespaces.forEach((namespace) =>
            {
                expect(namespace.sendNotification).to.have.been.called.with(NOTIFICATION_NAME)
            })
        })
        ...
        it("Resolves once all notifications are sent", async () =>
        {
            ...
            {
                const namespace = server.createNamespace(namespaceName)
                namespace.sendNotification = async () =>
                {
