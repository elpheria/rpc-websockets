...
import {
    ...
    TRpcNotificationParams, TRpcRequestHandler, TRpcRequestName
...
export default class Server extends EventEmitter
{
    ...
    private _namespaces: Map<TNamespaceName, Namespace>;
    private _sockets: Map<TJsonRpcSocketId, JsonRPCSocket>;
    ...
    _startServer(options: ServerOptions): WebSocketServer
    {
        ...
        server.on("connection", (socket: TCommonWebSocket, request) =>
        {
            this.emit("connection", socket, request)

            const u = url.parse(request.url, true)
            const ns = u.pathname
            const id: string = u.query[this.options.idParam] as string || uuid.v1()
     ...
     * @returns {Namespace}
     */
    createNamespace(name: TNamespaceName)
    {
     ...
     * @returns {Boolean}
     */
    hasNamespace(name: TNamespaceName)
    {
     ...
     */
    getNamespace(name: TNamespaceName)
    {
        assertNamespaceName(name)
     ...
     * @returns {Namespace}
     */
    getOrCreateNamespace(name: TNamespaceName)
    {
     ...
     * @return {Undefined}
     */
    closeNamespace(name: TNamespaceName)
    {
        if (this.hasNamespace(name))
        {
    ...
    _changeSubscriptionStatus(
        action: "on" | "off" | "once",
        isInternal: boolean,
        subscriptions: TRpcNotificationName | {[key: string]: TServerNotificationHandler},
        handler?: TServerNotificationHandler
    )
    {
     ...
     * @return {Undefined}
     */
    registerNotification(names: TRpcNotificationName | Array<TRpcNotificationName>, ns = "/")
    {
     ...
     * @returns {void}
     */
    unregisterNotification(names: TRpcNotificationName | Array<TRpcNotificationName>, ns = "/")
    {
     ...
     */
    onNotification(
        notification: TRpcNotificationName | {[key: string]: TServerNotificationHandler},
        handler?: TServerNotificationHandler
    )
    {
     ...
     * @returns {void}
     */
    registerMethod(name: TRpcRequestName, fn: TRpcRequestHandler, ns = "/")
    {
     ...
     * @returns {void}
     */
    unregisterMethod(name: TRpcRequestName, ns = "/")
    {
        if (this.hasNamespace(ns))
     ...
     * @returns {void}
     */
    registerInternalMethod(name: TRpcRequestName, fn: TRpcRequestHandler, ns = "/")
    {
     ...
     * @returns {void}
     */
    unregisterInternalMethod(name: TRpcRequestName, ns = "/")
    {
        if (this.hasNamespace(ns))
     ...
     * @deprecated
     */
    register(name: TRpcRequestName, fn: TRpcRequestHandler, ns = "/")
    {
     ...
     * @deprecated
     */
    event(name: TRpcNotificationName, ns = "/")
    {
        assertArgs(arguments, {
            "event": "string",
            "[ns]": "string"
